		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	int rc;
 	if (sscanf(buf, "%d", &data) != 1)
		return -EINVAL;
 	if (data < hap->vtg_min) {
		pr_err("%s: mv %d not in range (%d - %d), using min.", __func__, data, hap->vtg_min, hap->vtg_max);
		data = hap->vtg_min;
	} else if (data > hap->vtg_max) {
		pr_err("%s: mv %d not in range (%d - %d), using max.", __func__, data, hap->vtg_min, hap->vtg_max);
		data = hap->vtg_max;
	}
 	hap->vmax_mv = data;
